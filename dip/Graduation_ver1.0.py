...
def Make_button(Frame):
    ...
    for i in range(0,48):
        ...
        if(little_array[i] == 'Space'):
            tkinter.Button(Frame,text = 'Space', overrelief="solid",command = key_func, width=3,repeatdelay=400, repeatinterval=80).grid(row=2,column = 11)
        elif(little_array[i] == 'Shift'):
            tkinter.Button(Frame,text = 'Shift', overrelief="solid",command = key_func, width=3,repeatdelay=400, repeatinterval=80).grid(row=3,column = 11)
        elif(little_array[i] == 'Erase'):
            tkinter.Button(Frame,text = 'Erase', overrelief="solid",command = key_func, width=3,repeatdelay=400, repeatinterval=80).grid(row=1,column = 12)
        else:
            tkinter.Button(Frame,text = little_array[i] + '( '+ big_array[i] +' )', overrelief="solid",command = key_func, width=3,repeatdelay=400, repeatinterval=80).grid(row=temp_row,column = temp_col)
            temp_col += 1
...
def Connect_wifi_double(event):
    ...
    tkinter.Button(frame_button, text = "접속", overrelief="solid", height = 1,width=8, command=Close).pack()
    ...
    tkinter.Button(frame_id_insert, text = "선택", overrelief="solid", height = 1,width=8, command=Ins).pack(side="left")
    tkinter.Button(frame_id_insert, text = "다시검색", overrelief="solid", height = 1,width=8, command=Retry).pack(side="right")
